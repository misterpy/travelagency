...
if ( ! function_exists( 'df_social_connect' ) ) {
    ...
    function df_social_connect() {
        ...
        $_is_customizing = df_is_customizing();
        $enable_sc = $inline_topbar_social_css = $output = '';

        if ( $_is_customizing ) {
            $enable_sc = true;
            $inline_topbar_social_css = ( $df_options['enable_topbar_social_icon'] == 1 ) ? 'style="display:table-cell;"' : 'style="display:none;"';
        } else {
            $enable_sc = ( $df_options['enable_topbar_social_icon'] == 1 );
        }

        if ( $enable_sc ) :

            foreach( $active_socials as $social_site ) {
                $df_social = isset( $df_options[ $social_site ] ) ? $df_options[ $social_site ] : null;
...
if ( ! function_exists( 'df_btn_shape' ) ) {
    ...
    function df_btn_shape( $classes = '' ) {
        ...
        $df_button_shape    = $df_options[ 'df_button_shape' ];
...
function df_password_form() {
        ...
        <p><input name="post_password" id="' . $label . '" type="password" size="20" placeholder="password" /> <input placeholder="password" type="submit" name="submit" value="' . esc_attr__('Submit') . '" /></p></form></div>';
        return $output;
    }
