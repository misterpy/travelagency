...
if (!function_exists('df_blog_class_for_infinite_pagination')) :
    ...
    function df_blog_class_for_infinite_pagination() {
        if ( is_singular() && is_archive() ) { return false; }
...
if (!function_exists( 'df_blog_grid_col' ) ) :
    function df_blog_grid_col() {
        ...
        $classes        = '';
        $df_blog_layout = is_archive() ? $df_options[ 'archive_layout' ] : $df_options[ 'blog_layout' ];
        $df_grid_col    = is_archive() ? $df_options[ 'blog_grid_col' ] : $df_options[ 'blog_grid_col' ];
...
if ( !function_exists( 'df_postmeta_loop_grid' ) ) :
    function df_postmeta_loop_grid() {
        ...
        $df_grid_type   = is_archive() ? $df_options[ 'arch_content_grid' ] : $df_options[ 'blog_content_grid' ];
        $df_bg_mode_lay = get_post_meta( get_the_ID(), 'df_metabox_ftr_img_background', true );
        $except_PF      = ( get_post_format() != 'aside' && get_post_format() != 'chat' && get_post_format() != 'status' && get_post_format() != 'quote' && get_post_format() != 'link' );
...
function df_category_blog() {
    if( !is_archive() ) :
        ...
        $categories_list = get_the_category_list( __( ', ', 'dahztheme' ) );
...
if ( !function_exists( 'df_share_elements' ) ) :
    function df_share_elements() {
        $df_options           = get_option( 'df_options' );
        $df_enable_share      = $df_options[ 'blog_share' ];

        if ( $df_enable_share != '1' ) : return; endif;

        $src                  = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ) );
        $df_enable_fb_blog    = $df_options[ 'fb_share' ];
        $df_enable_twit_blog  = $df_options[ 'twt_share' ];
        $df_enable_gp_blog    = $df_options[ 'gplus_share' ];
        $df_enable_pin_blog   = $df_options[ 'pin_share' ];
        $df_enable_email_blog = $df_options[ 'mail_share' ];
        $df_enable_ldn_blog   = $df_options[ 'link_share' ]; ?>
...
if ( !function_exists( 'df_blog_share' ) ) :
    function df_blog_share() {
        $df_options     = get_option( 'df_options' );
        $enable_lk_btn  = $df_options[ 'love_btn' ] == '1';
        $df_blog_layout = is_archive() ? $df_options[ 'archive_layout' ] : $df_options[ 'blog_layout' ];
...
if (!function_exists('df_blog_content')) :
    ...
    function df_blog_content(){
        ...
        $df_blog_layout  = is_archive() ? $df_options[ 'archive_layout' ] : $df_options[ 'blog_layout' ];
        $df_grid_type    = is_archive() ? $df_options[ 'arch_content_grid' ] : $df_options[ 'blog_content_grid' ];
        $df_full_summary = is_archive() ? $df_options[ 'arch_content_list' ] : $df_options[ 'blog_content_list' ];
...
if (!function_exists('df_custom_excerpt_more')) :
    function df_custom_excerpt_more( $more ) {
        ...
        $more .= __('Continue reading', 'dahztheme');
        $more .= '</a>';
...
if (!function_exists('df_isotope_category_blog')) :
    ...
    function df_isotope_category_blog() {
        ...
        $df_enable_filter_cat = $df_options[ 'cat_filter' ];
        $df_homepage_layout   = $df_options[ 'blog_layout' ];
...
if (!function_exists('df_blog_author')) :
    ...
    function df_blog_author() {
        ...
        $html .= '<h4 class="title-about">' . __('About ', 'dahztheme') . '<a href="' . get_author_posts_url(get_the_author_meta('ID')) . '">' . get_the_author() . '</a></h4>';
...
if (!function_exists('df_single_blog_related_post')) :
    ...
    function df_single_blog_related_post() {
        ...
        echo '<h3 class="related-post-title">' . __('You May Also Like', 'dahztheme') . '</h3>';
