...
class Df_Mega_Menu {
	...
	function add_custom_nav_fields( $menu_item ) {
		...
		$menu_item->df_btn_enabled = get_post_meta( $menu_item->ID, '_menu_item_df_btn_class_enabled', true );
	...
	function update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
		...
		// Link button enabled
		$enable_btn_link = isset($_REQUEST['menu-item-df-button-class'], $_REQUEST['menu-item-df-button-class'][$menu_item_db_id]);
		update_post_meta( $menu_item_db_id, '_menu_item_df_btn_class_enabled', $enable_btn_link );
