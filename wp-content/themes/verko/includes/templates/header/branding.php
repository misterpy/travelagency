...
$df_logo_alt 			        	= $df_options['logo_alt'];
$df_logo_light 			      	= $df_options['logo_light'];
$df_logo_dark 				      = $df_options['logo_dark'];
...
$meta_header_transparency   = get_post_meta( $page_id, 'df_metabox_header_transparency', true );
 ...
 */
	...
	if ( ( $meta_header_transparency ) && ( $meta_header_transparency != 'default' ) ) {
		$navbar_transparency_class = 'nav-transparent';
		$navbar_transparency_scheme = get_post_meta( $page_id, 'df_metabox_header_transparency', true );
	}
	...
	function logoChange(){
		...
		if ((windowWidth > 1050 || windowWidth < 800) || !dfGlobals.isMobile) {
			$(window).scroll(function() {
			    if ($(this).scrollTop() > 1){
			       <?php if( $df_logo_alt != '') { ?>
		             $('.normal-logo').attr('src', '<?php echo esc_url($df_logo_alt) ?>' );
			       <?php } ?>
			    } else {
			    	<?php if( $df_logo_alt != '') { ?>
		             $('.normal-logo').attr('src', '<?php echo esc_url($logo) ?>' );
			        <?php } ?>
			    }
			});
		}/*IF window Width*/
