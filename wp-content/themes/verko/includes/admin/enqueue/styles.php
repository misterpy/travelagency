...
if (!function_exists('df_load_frontend_css')) {
    ...
    function df_load_frontend_css() {
        ...
        $navbar_font_family             = isset( $df_options[ 'nav_font_family' ] ) ? $df_options[ 'nav_font_family' ] : 'Karla';
        $navbar_font_family_query       = str_replace( ' ', '+', $navbar_font_family );
        $navbar_font_weight_and_style   = isset( $df_options[ 'nav_font_weight' ] ) ? $df_options[ 'nav_font_weight' ] : '400';
        ...
        $headings_font_family           = isset( $df_options[ 'heading_font_family' ] ) ? $df_options[ 'heading_font_family' ] : 'Dosis';
        $headings_font_family_query     = str_replace( ' ', '+', $headings_font_family );
        $headings_font_weight_and_style = isset( $df_options[ 'heading_font_weight' ] ) ? $df_options[ 'heading_font_weight' ] : '400';
        ...
        $body_font_family               = isset( $df_options[ 'body_font_family' ] ) ? $df_options[ 'body_font_family' ] : 'Karla';
        $body_font_family_query         = str_replace( ' ', '+', $body_font_family );
        $body_font_weight_and_style     = isset( $df_options[ 'body_font_weight' ] ) ? $df_options[ 'body_font_weight' ] : '400';
        ...
        wp_enqueue_style('df-layout', CHILD_THEME_URI . '/includes/assets/css/layout'.$suffix.'.css', NULL, NULL);

        if (is_rtl()) {
            wp_enqueue_style('rtl', ASSETS_URI . 'css/rtl'.$suffix.'.css', NULL, NULL);
        }
...
if (!function_exists('df_enqueue_customizer_admin_style')) {
    ...
    function df_enqueue_customizer_admin_style() {
        $suffix = dahz_get_min_suffix();
        wp_enqueue_style('df-customizer-admin', ASSETS_URI . 'css/admin/theme-customizer'.$suffix.'.css');
    }
